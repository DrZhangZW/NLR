...
def visualize_tensors(tensor_list, save_dir, epoch, iteration, titles='train', rank=0):
    """
    可视化多个张量并自动命名保存（使用最大batch size保存）

    参数:
        tensor_list: 包含张量的列表 [underE.tensors, pred_img, under.tensors]
        save_dir: 保存目录路径
        epoch: 当前训练轮次
        iteration: 当前迭代次数
        titles: 可选的自定义标题列表
    ...
    # ========== 1. 张量预处理（GPU -> CPU，避免显存占用） ==========
    processed_tensors = []
    for idx, tensor in enumerate(tensor_list):
        # 1. 脱离计算图 + 转到CPU + 转为float（避免半精度/整型问题）
        t = tensor.detach().cpu().float()
        # 2. 处理维度：确保是4D张量 [B, C, H, W]（兼容单张图片B=1的情况）
        if len(t.shape) == 3:
            t = t.unsqueeze(0)
        # 3. 裁剪/归一化防护：避免极值导致可视化异常
        t = (t - t.min()) / (t.max() - t.min() + 1e-8)  # 防止除零
        t = torch.clamp(t, 0.0, 1.0)  # 最终限制在[0,1]
        processed_tensors.append(t)
    
    # ========== 2. 按最大batch size对齐张量（不足部分用空白填充） ==========
    # 找到最大batch size
    max_bs = max([t.size(0) for t in processed_tensors])
    aligned_tensors = []
    
    for t in processed_tensors:
        current_bs = t.size(0)
        if current_bs < max_bs:
            # 计算需要填充的数量
            pad_bs = max_bs - current_bs
            # 创建空白填充张量（与原张量同shape，值为1.0即白色）
            padding = torch.ones((pad_bs, t.size(1), t.size(2), t.size(3)), dtype=t.dtype)
            # 拼接填充张量
            t_padded = torch.cat([t, padding], dim=0)
            aligned_tensors.append(t_padded)
        else:
            # 如果batch size等于最大值，直接使用
            aligned_tensors.append(t[:max_bs])
    
    # ========== 3. 张量拼接与保存 ==========
    # 按列拼接（每个张量占一列）
    combined = torch.cat(aligned_tensors, dim=0)
    
    # 网格布局计算
    num_tensors = len(aligned_tensors)
    bs_per_tensor = max_bs  # 每行显示一个batch的图片（使用最大batch size）
    nrow = bs_per_tensor    # 每行显示batch_size张图片
    ncol = num_tensors      # 每列显示一种类型的张量（Input/Pred/GT等）

    filename = f"{titles}_epoch_{int(epoch):04d}_iter_{int(iteration):06d}_rank_{int(rank):02d}.png"
    save_path = os.path.join(save_dir, filename)
    
    # 可视化保存（normalize=True自动将[0,1]映射到[0,255]）
    vutils.save_image(
        combined,
        save_path,
        nrow=nrow,
        normalize=True,  # 关键：确保数值范围正确
        scale_each=True,  # 每个图片独立归一化
        pad_value=1.0,  # 分隔线用白色（0是黑色，对比更明显）
        padding=2  # 增加分隔线宽度，提升可读性
    )
    print(f"[Rank {rank}] 可视化图片已保存到统一目录: {save_path} (最大Batch Size: {max_bs})")
