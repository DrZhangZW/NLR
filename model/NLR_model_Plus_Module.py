...
class SubproblemSolver(nn.Module):
    ...
    # ===================== 纯物理Retinex分解函数（替代CTDM_retinex）=====================
    def physical_retinex_decompose(self, feat, mask=None, eps=1e-6):

        # 1. 基础维度获取+语法错误修复（原代码x.shape改为feat.shape）
        B, C, H, W = feat.shape
        device, dtype = feat.device, feat.dtype

        # 2. 掩码预处理：尺寸自适应+通道扩展+有效区域转换，兼容无掩码场景
        valid_mask = torch.ones((B, C, H, W), device=device, dtype=dtype)  # 兜底：无掩码时全区域有效
        if mask is not None:
            # 自动补维为4维[B,1,H,W]，适配F.interpolate输入要求
            if len(mask.shape) == 3:
                mask = mask.unsqueeze(1)
            # 插值缩放到特征图空间尺寸[H,W]，近邻插值保证掩码二值性不丢失
            mask_resize = F.interpolate(mask.float(), size=[H, W], mode='nearest')
            # 转换为有效区域掩码（1=有效，0=无效）+ 扩展为特征图通道数C，自动适配3/32通道
            valid_mask = (1.0 - mask_resize).expand_as(feat).to(dtype=dtype)

        # 3. Retinex核心分解：严格遵循物理公式，R/L均乘有效掩码过滤无效区域
        # 光照L：通道维度最大值（Retinex核心），单通道[B,1,H,W]→扩展为[B,C,H,W]适配特征图
        L = torch.max(feat, dim=1, keepdim=True)[0]
        L = L.expand_as(feat)
        # 反射率R：特征/光照（加eps防除零），R/L均与有效掩码相乘，同步过滤无效区域
        R = feat / (L + eps) * valid_mask
        L = L * valid_mask  # 补充：光照L同步乘有效掩码，与反射率R过滤逻辑一致

        # 4. 数值约束：与Relight模块光照/反射率范围严格对齐，避免极端值导致训练震荡
        L = torch.clamp(L, min=0.01, max=1.0)  # 光照范围与Relight内部约束一致
        R = torch.clamp(R, min=0.0, max=1.0)  # 反射率物理范围0~1，符合Retinex理论

        return R, L



    def process_ctdm2(self, low_feat, high_feat, mask, mask_y, stage):
        """
        最终解耦版：
        1. Relight分支梯度完全封闭在CTDM内部，不优化/不干扰基础网络
        2. CTDM输出与Retinex/Relight解耦的独立变量（仅数值传递，无梯度）
        3. 保留stage2/3阶段化Relight训练，适配配对/非配对数据
        4. 特征计算贴合Retinex物理规律，提取高质量解耦特征
        5. 完善显存释放，避免浪费
        """
        # ========== 0. 基础配置 ==========
        device = low_feat.device
        is_paired = False if stage >= 3 else True  # 前期配对，后期非配对
        relight_weight = 0.1 if stage == 2 else 0.2  # stage2轻量训练，避免过拟合

        # ========== 1. 梯度隔离：核心不变，保护原始low_feat/基础网络 ==========
        low_feat_for_loss = low_feat.detach().clone()
        high_feat_for_loss = high_feat.detach().clone() if high_feat is not None else None

        # ========== 2. Retinex反射率/亮度分解（仅用无梯度副本，CTDM分解不更新） ==========
        low_R, low_L = self.CTDM_retinex(low_feat_for_loss, mask)
        high_R, high_L = self.CTDM_retinex(high_feat_for_loss, mask_y) if high_feat_for_loss is not None else (None,
                                                                                                               None)

        # ========== 3. CTDM分解损失（不变，约束Retinex分解的物理有效性） ==========
        loss_low = loss_CTDM(low_feat_for_loss, low_R, low_L, mask)
        loss_high = torch.tensor(0.0, device=device)
        if is_paired and high_R is not None and high_L is not None and high_feat_for_loss is not None:
            loss_high = loss_CTDM(high_feat_for_loss, high_R, high_L, mask_y)
        ctdm_total_loss = loss_low + loss_high

        # ========== 4. Relight光照交换损失（梯度封闭在CTDM内部，核心修改） ==========
        relight_loss = torch.tensor(0.0, device=device)
        # 新增：定义解耦输出的临时变量，初始为原始无梯度副本（兼容无Relight的场景）
        ctdm_out_feat = low_feat_for_loss  # 无Relight时，直接返回原始分解特征
        if (stage == 2 or stage == 3) and high_L is not None:
            # 步骤1：high_L梯度隔离+高斯模糊，提前去除残差，避免引入伪影
            high_L_detach = high_L.detach().clone()
            high_L_detach = gaussian_blur_2d(high_L_detach, kernel_size=3, sigma=1.0)
            high_L_detach = high_L_detach.expand_as(low_L)

            # 步骤2：拼接特征梯度隔离，确保Relight不干扰Retinex分解的low_L
            concat_feat = torch.cat((low_L.detach(), high_L_detach), dim=1)

            # 步骤3：Relight前向输出（无detach，梯度完整，仅用于更新Relight自身）
            relight_out = self.relight(concat_feat)

            # 步骤4：计算Relight损失（梯度仅回传至Relight模块，核心：入参全隔离）
            relight_loss = loss_relight(
                low_R.detach(),
                low_L.detach(),
                relight_out,
                ...
                paired=is_paired,
                mask=mask,
                stage=stage  # 核心补充：传递stage参数，让loss_relight的阶段化逻辑生效
            ) * relight_weight

            # 步骤5：核心！计算CTDM解耦输出特征（贴合Retinex物理规律：反射率×优化后光照）
            # low_R.detach()：反射率无梯度，仅提供纹理信息；relight_out：优化后光照，保平滑
            ctdm_out_feat = low_R.detach() * relight_out

            # 步骤6：及时释放Relight相关临时张量，减少显存占用（关键：包含relight_out）
            del high_L_detach, concat_feat, relight_out

        # ========== 5. 最终梯度隔离：CTDM输出完全解耦，无任何梯度回传 ==========
        # 核心：detach()后仅传递数值，Relight/Retinex的梯度完全封闭在CTDM内部，不污染基础网络
        low_feat = ctdm_out_feat.detach()

        # ========== 6. 总损失 + 彻底释放所有CTDM内部张量 ==========
        total_loss = ctdm_total_loss + relight_loss
        del low_feat_for_loss, high_feat_for_loss, low_R, low_L, high_R, high_L, ctdm_out_feat

        # ========== 7. 返回解耦后的独立变量（仅数值，无梯度） ==========
        return low_feat, total_loss
